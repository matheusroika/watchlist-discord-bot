...
export default async function handleListenedMessage(
  message: Discord.Message,
  { prefix, language }: Config
) {
  const {
    addCommand,
    listenedMessage,
    common,
  }: LanguageFile = require(`../../languages/${language}.json`);
  ...
  if (
    ...
    if (!!!media.overview) {
      const mediaDataInEnglish = (await getMedia(
    ...
    if (searchTitle === mediaOriginalTitle) {
      ...
      mediaEmbed
        ...
        .addFields(
          { name: "** **", value: "** **" },
          {
            name: listenedMessage.foundSimilar.searched,
            value: searchTitle,
            inline: true,
          },
          {
      ...
      message.channel.send(mediaEmbed).then((message) => {
        message.react("✅");
        message.react("❌");
          ...
          .then(async (collected) => {
            ...
            if (reaction?.emoji.name === "✅") {
              ...
              mediaEmbed
                ...
                .setURL(
                  `https://www.themoviedb.org/${media.media_type}/${media.id}`
                )
                .setThumbnail(
                  `${images.secure_base_url}/${images.poster_sizes[4]}/${media.poster_path}`
                )
              ...
              mediaEmbed
                .setTitle(listenedMessage.title)
                .setDescription(
                  Mustache.render(listenedMessage.cancelled, [prefix])
                );
              message.reactions.removeAll();
              message.edit(mediaEmbed);
            }
